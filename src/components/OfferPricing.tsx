...
import { useSalesCount } from '../hooks/useSalesCount';

const OfferPricing: React.FC = () => {
  const { t } = useTranslation();
  const { spotsRemaining, isEarlyAccess, showCounter } = useSalesCount();
    ...
    <section id="pricing" className="py-20 bg-gradient-to-br from-rose-900 to-pink-900 text-white relative overflow-hidden">
      ...
      <div className="max-w-5xl mx-auto px-4 relative">
        <div className="text-center mb-16">
          ...
          {showCounter && (
            <div className="mb-6">
              <p className="text-xl text-white font-bold text-shadow-glow">
                {t('earlyAccess.spotsLeft', { count: spotsRemaining })}
              </p>
            </div>
          )}
          ...
          {isEarlyAccess && (
            <div className="bg-white/15 backdrop-blur-sm rounded-2xl p-12 border-2 border-emerald-400 shadow-2xl relative">
              ...
              <div className="text-center mb-12">
                <div className="mb-4">
                  <span className="text-6xl md:text-7xl font-bold text-white text-shadow-strong">{t('pricing.earlyAccessPrice')}</span>
                </div>
              ...
              <div className="grid md:grid-cols-2 gap-8 mb-12">
                <div className="bg-white/10 rounded-2xl p-8 text-center border-2 border-green-400 shadow-lg">
                  <div className="text-green-400 font-bold text-sm mb-2">MEJOR VALOR</div>
                  <div className="flex items-center justify-center mb-4">
                    <span className="text-3xl font-bold text-white">{t('pricing.earlyAccessPrice')}</span>
                  </div>
                  <p className="text-white font-semibold">Pago único</p>
                </div>
                <div className="bg-white/10 rounded-2xl p-8 text-center shadow-lg">
                  <div className="flex items-center justify-center mb-4">
                    <span className="text-3xl font-bold text-white">{t('pricing.earlyAccessMonthly')}</span>
                    <span className="text-white/80 ml-1">/mo</span>
                  </div>
                  <p className="text-white font-semibold">3 pagos mensuales</p>
                  <p className="text-white/80 text-sm">Comienza hoy por solo {t('pricing.earlyAccessMonthly')}</p>
                </div>
              </div>
              ...
              <div className="bg-gradient-to-r from-yellow-400/20 to-orange-400/20 rounded-2xl p-10 border border-yellow-400/30 shadow-lg">
                <div className="flex items-center justify-center mb-6">
                  ...
                  <span className="text-xl font-bold text-white text-shadow-strong">Bono de Fundadora Incluido</span>
                </div>
                <p className="text-center text-white font-semibold text-shadow-glow text-lg">
                  Baraja de Chequeo Emocional™ (Valor: $47) - GRATIS
                </p>
              </div>
            </div>
          )}
          
          {/* Standard Card - Only show if sales > 300 */}
          {!isEarlyAccess && (
            <div className="bg-white/15 backdrop-blur-sm rounded-2xl p-12 border border-white/30 shadow-2xl">
              ...
              <div className="bg-white/10 rounded-2xl p-8 text-center shadow-lg">
                ...
                <p className="text-white font-semibold">3 pagos mensuales</p>
                <p className="text-white/80 text-sm">Comienza hoy por solo {t('pricing.standardMonthly')}</p>
              </div>
              </div>
            </div>
          )}
